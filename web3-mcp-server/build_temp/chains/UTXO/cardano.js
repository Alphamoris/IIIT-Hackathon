...
async function initLucid() {
    ...
    try {
        ...
        const provider = new Blockfrost(`https://cardano-${CARDANO_NETWORK}.blockfrost.io/api/v0`, BLOCKFROST_API_KEY);
...
export async function getWalletInfo() {
    try {
        ...
        if (utxos.length > 0) {
            ...
            const value = utxos.reduce((acc, utxo) => acc.add(utxo.assets), 
...
export async function sendAda(recipientAddress, amountAda, metadata = null) {
    try {
        // Validate input
        if (!recipientAddress) {
...
export async function sendTokens(recipientAddress, policyId, assetName, amount, adaAmount = null) {
    try {
        // Validate input
        if (!recipientAddress) {
            throw new Error('Recipient address is required');
...
export function registerCardanoTools(server) {
    // Get Address Balance
    server.tool("getCardanoAddressBalance", "Get balance and token holdings for a Cardano address", {
        ...
        try {
            ...
            const tokenList = tokens.length > 0
                ? tokens.map(token => `${token.quantity} ${token.assetName || token.unit} (Policy: ${token.policyId.substring(0, 8)}...)`).join('\n')
                : 'No tokens found';
            // Build response
            return {
                content: [
...
${tokens.length > 0 ? `Token Holdings (${tokens.length}):\n${tokenList}` : 'No token holdings found'}`,
    ...
    server.tool("getCardanoWalletInfo", "Get the current wallet information, including balance and tokens", {}, async () => {
        try {
            // Get wallet info
            const walletInfo = await getWalletInfo();
            // Format token list
            const tokenList = walletInfo.tokens.length > 0
                ? walletInfo.tokens.map((token) => `${token.quantity} ${token.name}`).join('\n')
                : 'No tokens found';
            return {
                content: [
                    {
