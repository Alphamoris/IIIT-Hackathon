...
async function createWalletContract(client, address, keyPair) {
...
export function registerTonTools(server) {
    // Get TON Balance
    ...
    server.tool("getTonTransactionHistory", "Get transaction history for a TON address", {
        ...
        try {
            ...
            const transactions = await withRetry(() => client.getTransactions(validAddress, { limit: limit }));
...
Latest Seqno: ${masterchainInfo.latestSeqno}`,
    ...
    server.tool("sendTonTransaction", "Send TON from your wallet to another address using mnemonic from .env", {
        ...
        try {
            ...
            const { contract: walletContract, walletVersion } = await createWalletContract(client, walletAddress, keyPair);
