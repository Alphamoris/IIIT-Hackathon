...
export function registerRippleTools(server) {
    // Get XRP Balance
    ...
    server.tool("getXrpTransactionHistory", "Get transaction history for an XRP address", {
        ...
        try {
            ...
            const txList = response.result.transactions.map((tx) => {
...
Amount: ${transaction.Amount.currency ?
    ...
    // Get XRP Ledger Info
    server.tool("getXrpLedgerInfo", "Get current XRP Ledger information", {}, async () => {
        try {
            const client = await getClient();
...
Note: Transaction was submitted successfully but validation status is pending.`,
    ...
    server.tool("getXrpTokenBalances", "Get token balances for an XRP address", {
        ...
        try {
            ...
            const tokenBalances = response.result.lines.map((line) => {
...
Explorer Link: ${XRP_EXPLORER}/transactions/${result.result.hash}`,
                ...
                throw new Error(`Failed to sign/submit transaction: ${signError.message}`);
            }
        }
        catch (err) {
            const error = err;
